...
def get_feeds():
    data = {
        "url": "https://aweme.snssdk.com/aweme/v1/feed/?type=0&max_cursor=0&min_cursor=0&count=6&volume=0.06666666666666667&pull_type=2&need_relieve_aweme=0&filter_warn=0&req_from&is_cold_start=0&js_sdk_version=1.2.2&app_type=normal&manifest_version_code=321&_rticket=" + str(
            int(
                time.time())) + "&ac=wifi&device_id=52650937206&iid=50374690113&os_version=8.1.0&channel=gray_3306&version_code=330&device_type=ONEPLUS%20A5000&language=zh&uuid=866265035315870&resolution=1080*1920&openudid=4617150637217100&update_version_code=3216&app_name=aweme&version_name=3.3.0&os_api=27&device_brand=OnePlus&ssmix=a&device_platform=android&dpi=420&aid=1128"}
...
while page < page_max:
    ...
    feeds = get_feeds()
    if len(feeds) > 0:
        print("page:" + str(page) + " feed len:" + str(len(feeds)))
        for feed in feeds:
            if feed['is_ads']:
